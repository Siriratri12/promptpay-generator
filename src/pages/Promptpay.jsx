...
// Formats digits as the user types: phone-style (3-3-4) while short,
// national-ID style (1-4-5-2-1) once it grows past 10 digits,
// e-Wallet style (grouped by 4) once it grows past 13 digits.
// Purely visual — sanitizeDigits() strips it back out before the
// payload is built, so it never affects the actual EMV data.
...
export default function App() {
  const { target: urlTarget, amount: urlAmountParam } = useParams();
  ...
  // If the URL is /promptpay/0971328795, pre-fill the field automatically.
  useEffect(() => {
    if (!urlTarget) return;
    const digits = sanitizeDigits(urlTarget).slice(0, 15);
    if (digits) setTarget(formatForDisplay(digits));
  }, [urlTarget]);

  // If the URL has /amount as a path segment or ?amount= as a query param,
  // pre-fill the amount field automatically (path segment takes priority).
  useEffect(() => {
    const urlAmount = urlAmountParam || searchParams.get("amount");
    if (!urlAmount) return;
    const n = Number(urlAmount);
    if (!isNaN(n) && n > 0 && n <= 999999.99) {
      setAmount(String(urlAmount));
    }
  }, [urlAmountParam, searchParams]);
  ...
  const handleTargetChange = (e) => {
    const digits = sanitizeDigits(e.target.value).slice(0, 15);
    setTarget(formatForDisplay(digits));
  };
